...
userSchema.methods.generateToken = function () {
  ...
  return jwt.sign({
    _id: this._id
  }, config.jwtPrivateKey, {expiresIn: '2h'})
}
...
function userValidator(data) {
  ...
  const schema = Joi.object({
    ...
    jurisdiction: Joi.string().valid('user', 'admin').messages({
      'string.base': ' jurisdiction 必须为string格式',
      'any.only': ' status 取值错误，可选值为: user, admin'
    }),
    _id: Joi.objectId()
  })
  // 调用函数传入 要校验的 data
